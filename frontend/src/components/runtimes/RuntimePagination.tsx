...
interface RuntimePaginationProps {
  currentPage: number;
  pageSize: number;
  total: number;
...
export function RuntimePagination({
  currentPage,
  pageSize,
  total,
  onPageChange,
    ...
    <div className="px-6 py-4 border-t border-slate-200/80 bg-slate-50/50 flex flex-col sm:flex-row items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-3 text-xs text-slate-600">
        <span>
          ...
          <strong className="font-semibold text-slate-900">{total}</strong> runtimes
        </span>

        <div className="hidden sm:flex items-center gap-1.5 pl-3 border-l border-slate-200">
          <label htmlFor="runtimePageSizeSelect" className="text-slate-500 text-xs">
            Per page:
          </label>
          <select
            id="runtimePageSizeSelect"
            value={pageSize}
